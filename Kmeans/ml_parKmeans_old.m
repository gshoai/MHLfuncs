function [Cs, idxs] = ml_parKmeans_old(D, k)
% ml_parKmeans_old: Parallel k-means
% Inputs:
...
% Some timing info for [d,n] = size(D), D is single precision
...
for iter=1:nMaxIter
    ...
    parfor j=1:n
        dist_j = c2 - 2*D(:,j)'*Cs;
        [minVals(j), idxs(j)] = min(dist_j);
        minVals(j) = minVals(j) + d2(j);
    end;  
